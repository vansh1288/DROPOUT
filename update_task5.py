...
class ProtocolBridge:
    ...
    async def _handle_dropout_notify(self, header: MessageHeader, payload: bytes):
        ...
        if len(round_state.dropout_clients) >= round_state.expected_clients - round_state.threshold:
    ...
    async def _check_round_completion(self, round_state: RoundState):
        ...
        if completed >= round_state.expected_clients - len(round_state.dropout_clients):
    ...
    async def _aggregate_and_broadcast(self, round_state: RoundState):
        ...
        for client_state in round_state.clients.values():
            if not client_state.completed and client_state.client_id not in round_state.dropout_clients:
                continue
...
'''
    with open(path, "w") as f:
        f.write(new_content)
    print("Updated protocol_bridge.py")

def update_shamir_recovery():
...
'''
    with open(path, "w") as f:
        f.write(new_content)
    print("Updated shamir_recovery.py")

if __name__ == "__main__":
    update_protocol_bridge()
    update_shamir_recovery()
